...
{
    public class OB940UploadFileController : Controller
    {
        ...
        /// <returns></returns>
          public ActionResult OB940UploadFile(string p_cmp_id, string p_str_scn_id)
        {
                        ...
                        using (var file_reader = new CsvFileReader(p_str_file_path))
                        {
                            while (file_reader.ReadRow(lst_csv_data))
                            {
                                ...
                                if (lst_csv_data[0].ToUpper().Equals("SLF940"))
                                {
                                    ...
                                    {
                                        objOB940UploadFileDtl.error_desc = objOB940UploadFileDtl.error_desc + "Style Data Not Found";
                                    }
                                    // Color
                                    ...
                                    {
                                        if (lst_csv_data[16].Trim().Length > 75)
                                        {
                                            objOB940UploadFileDtl.itm_name = lst_csv_data[16].Trim().ToUpper().Substring(0, 75);
                                        }
                                        else
                                        {
                                            objOB940UploadFileDtl.itm_name = lst_csv_data[16].Trim().ToUpper();
                                        }

                                    }
                                    else
                                    {
                                        objOB940UploadFileDtl.itm_name = "-";
                                    }
                ...
                for (int j = 0; j < objOB940SBS.lstOB940SBSDtl.Count; j++)
                {
                    if (objOB940SBS.lstOB940SBSDtl[j].po_num == l_str_po_num)
                    {
                       ...
                       // objItemMaster.itm_name = objOB940UploadFileDtlLine.itm_name;
                        ...
                        //if (objOB940UploadFileDtl.error_desc.Length > 0)
                        if ((objOB940UploadFileDtl.error_desc.Length > 0) && (objOB940UploadFileDtl.error_desc.Contains("STYLE NOT FOUND") == false))
                        {
                            OB940InvalidData objOB940InvalidData = new OB940InvalidData();
