...
{
    public class xls_bl_vas_bill : IDisposable
    {
        ...
        public xls_bl_vas_bill(string templateFile)
        {
        ...
        public void PopulateHeader(string p_str_cmp_id, string p_str_bill_doc_id, string p_str_bill_dt, string p_str_bill_by, decimal p_dbl_tot_amount)
        {
            ...
            this.Names["__DataRptHeader"].Value = p_str_cmp_id + " - " + "VAS BILL DOCUMENT REPORT - BY " + p_str_bill_by;
        ...
        public void PopulateData(DataTable data, decimal p_dbl_tot_amount, bool loadImage = false)
        {
            if (data == null)
            {
                return;
            ...
            for (int i = 0; i < rowSize; i++)
            {
                ...
                this.Names["__Data_VASId"].Value = dr["ship_doc_id"];
                this.Names["__DATA_ShipDate"].Value = dr["ship_dt"];
                this.Names["__DATA_RptHdrNotes"].Value = dr["hdr_notes"];
                this.Names["__DATA_Line"].Value = dr["dtl_line"];
                this.Names["__DATA_RateId"].Value = dr["ship_itm_num"];
                this.Names["__DATA_RateIdDesc"].Value = dr["itm_name"];
                this.Names["__DATA_Notes"].Value = dr["notes"];
                this.Names["__DATA_Ctns"].Value = dr["ship_ctns"];
                this.Names["__DataUoM"].Value = dr["so_qty_uom"];
                this.Names["__DATA_Rate"].Value = dr["so_itm_price"];
                l_dec_amt = Convert.ToDecimal(dr["ship_ctns"]) * Convert.ToDecimal(dr["so_itm_price"]);
                this.Names["__DATA_Amount"].Value = l_dec_amt;
                rowIdx = this.InsertRowPaste(this.Names["__TEMPLATE_Data"], rowIdx);
            }

            this.Names["__Data_TotalBillAmt"].Value = p_dbl_tot_amount;
            rowIdx = this.InsertRowPaste(this.Names["__Data_Footer"], rowIdx);
        }
