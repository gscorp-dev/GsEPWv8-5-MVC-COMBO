...
{
    public class xls_IB_943_ACK : IDisposable
    {
        ...
        public xls_IB_943_ACK(string templateFile)
        {
        ...
        public void PopulateHeader(string p_str_cmp_id)
        {
            ...
            this.Names["__DataRptHeader"].Value = p_str_cmp_id + " - " + "INBOUND ACKNOWLEDGEMENT - REPORT ";
        ...
        public void PopulateData(DataTable data)
        {
            ...
            string l_str_prev_ib_doc_id = string.Empty;
            string l_str_cur_ib_doc_id = string.Empty;
            string l_str_perv_so_num = string.Empty;
            string l_str_ib_doc_dt = string.Empty;
            string l_str_cntr_id = string.Empty;
            string l_str_req_num = string.Empty;
            string l_str_eta_dt = string.Empty;
            string l_str_vendor_id = string.Empty;
            ...
            for (int i = 0; i < rowSize; i++)
            {
                ...
                l_str_cur_ib_doc_id = dr["ib_doc_id"].ToString();

                if (l_str_cur_ib_doc_id != l_str_prev_ib_doc_id)
                {

                    this.Names["__HDATA_ib_doc_id"].Value = dr["ib_doc_id"];
                    this.Names["__HDATA_ib_doc_dt"].Value = dr["ib_doc_dt"];
                    this.Names["__HDATA_cntr_id"].Value = dr["cntr_id"];
                    this.Names["__HDATA_req_num"].Value = dr["req_num"];
                    this.Names["__HDATA_eta_dt"].Value = dr["eta_dt"];
                    this.Names["__HDATA_vend_id"].Value = dr["vend_id"];

                    rowIdx = this.InsertRowPaste(this.Names["__Data_group_hdr"], rowIdx, 1, 3);
                }
                ...
                l_str_prev_ib_doc_id = l_str_cur_ib_doc_id;
            }
            this.Names["__Data_TotalCtns"].Value = l_itn_tot_ctns;
            this.Names["__Data_TotalCube"].Value = l_dec_totcube;
            this.Names["__Data_TotalWeight"].Value = l_dec_totwgt;
            rowIdx = this.InsertRowPaste(this.Names["__Data_Footer"], rowIdx, 1, 2);
        }
